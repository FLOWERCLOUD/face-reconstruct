...
def k_gabor_kernel_theta(theta, u=1.8, v=2.4):
    ...
    for y in range(0, filter_size):
        for x in range(0, filter_size):
            ...
            gaber[y, x] = exp(
                -0.5 * (x_theta * x_theta / sigma_u / sigma_u + y_theta * y_theta / sigma_v / sigma_v)) * cos(
                2 * pi * y_theta / lamda)
    return gaber
...
def get_response_map(img, theta_array, gabor_para):
    ...
    max_response = max_response.reshape(max_response.size, 1)
    for i in range(0, len(theta_array)):
        g_kernel = cv2.getGaborKernel((gabor_para['ksize'], gabor_para['ksize']), gabor_para['sigma'],
                                      theta_array[i] * pi / 180.0, gabor_para['lambda'], gabor_para['gamma'],
                                      gabor_para['psi'], ktype=cv2.CV_32F)
...
def get_orientation_map_cv_new(img_path, input_seg_img_path, write_dir, scale=0.5):
    ...
    filtered_img_array, max_response, max_orientaion_idx, g_kernel_array = get_response_map(img, theta_array,
                                                                                            gabor_para)
    for i in range(0, len(filtered_img_array)):
        ...
        cv2.imwrite(write_dir + 'g_kernel_image_'
                    + 'ksize' + str(gabor_para['ksize']) + 'sigma_' + str(gabor_para['sigma']) + 'theta_' + str(
            theta_array[i])
                    + 'lambda_' + str(gabor_para['lambda']) + 'gamma_' + str(gabor_para['gamma']) + 'psi_' + str(
            gabor_para['psi'])
                    + '.png', g_kernel)
    ...
    cv2.imwrite(write_dir + 'coeff_map_'
                ...
                + str(gabor_para['gamma']) + 'psi_' + str(gabor_para['psi'])
                + '.png', coeff_map)
    orientation_img = get_Orientationmap_color(max_orientaion_idx, coeff_map, img_mask, theta_array,
    ...
    orientation_img = get_Orientationmap_color(max_orientaion_idx, coeff_map, img_mask,
                                               filter_threshold=filter_threshold)
    print 'get_Orientationmap_color', time() - t1
...
def strand_trace(pickel_input_path, output_file_dir):
    ...
    if 0:
        ...
        strand_tracer.write_strand_to_image(output_file_dir + '/strand_with_color/', idx=-1, stran_len_threshold=1,
                                            use_theta_color=True, show_seed=False)
        strand_tracer.write_strand_to_image(output_file_dir + '/strand_with_color/', idx=-1, stran_len_threshold=3,
                                            use_theta_color=True, show_seed=False)
        strand_tracer.write_strand_to_image(output_file_dir + '/strand_with_color/', idx=-1, stran_len_threshold=10,
                                            use_theta_color=True, show_seed=False)
        strand_tracer.write_strand_to_image(output_file_dir + '/strand_with_color/', idx=-1, stran_len_threshold=30,
                                            use_theta_color=True, show_seed=False)
        strand_tracer.write_strand_to_image(output_file_dir + '/strand_with_color/', idx=-1, stran_len_threshold=50,
                                            use_theta_color=True, show_seed=False)
...
class Strand_Trace:
    ...
    def write_orientation_image(self, file_dir, coeff_threshold=0.07):
        ...
        cv2.imwrite(file_dir + '/orientaion_image_' + 'coeff_threshold_' + str(coeff_threshold) + '.png',
                    orientaion_image)
    ...
    def write_strand_to_image(self, file_dir, idx=0, stran_len_threshold=5, use_theta_color=False, show_seed=True):
        ...
        if idx > -1 and idx < len(self.strands):
            ...
            cv2.imwrite(file_dir + '/strand_' + str(idx) + 'strandlen' + str(len(strand.strand)) + '_thres_' + str(
                stran_len_threshold) + '_version2_' + '.png',
                        strand_image)
        elif idx == -1:
            strand_image = np.zeros((self.coeff_image.shape[0], self.coeff_image.shape[1], 3), dtype=np.uint8)
            ...
            cv2.imwrite(file_dir + '/strand_' + str(idx) + '_thres_' + str(stran_len_threshold) + '_version2_' + '.png',
                        strand_image)
...
class Stroke:
    ...
    def resolve_dir(self):
        ...
        dir_2d = end_point - start_point
        if np.linalg.norm(dir_2d) < 0.00001:
            self.intrinsic_2d_direction[self.stroke.shape[0] - 1, :] = self.intrinsic_2d_direction[
                                                                       self.stroke.shape[0] - 2, :]
...
def test_img_mesh():
    img_mask = cv2.imread('E:/workspace/dataset/hairstyles/2d_hair/result/A1301043736980A/' + 'img_mask_.png',
...
if __name__ == '__main__':
    ...
    if 0:
        ...
        # buk = ['A13010438114107']
        buk = ['A1301043678290A', 'A1301043736980A', 'A1301043828500A', 'A13010436665609', 'A13010436691206',
               'A13010436840808',
