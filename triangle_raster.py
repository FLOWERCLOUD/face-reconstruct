...
class MetroMesh(object):
    ...
    def __init__(self):
        ...
        # self.FFP_V = np.array([0],dtype=np.int32) #use for vertex face
        pass
    def set_mesh(self,v,vertex_color=np.array([]),normal=np.array([]),vt=np.array([]),face=np.array([]),n_face =np.array([]),t_face=np.array([])):
        self.v = v.copy()
        ...
        for x in range(0,mip.shape[1]):
            if ifequal(p[y*2,x*2],bkg):
                p[y*2,x*2] = mean4Pixelw( mip[y,x,:],144,
                                       ...
                                       mip[y - 1, x-1] if x > 0 and y >0  else bkg, 16 if x >0 and y >0 else 0)
            if ifequal(p[y*2,x*2+1],bkg):
                p[y*2,x*2+1] = mean4Pixelw( mip[y,x,:],144,
                                       mip[y,x+1] if x < mip.shape[1] -1 else bkg, 48 if x <mip.shape[1]-1 else 0,
                                       mip[y-1, x ] if y > 0 else bkg, 48 if y > 0 else 0,
            ...
            if ifequal(p[y*2+1,x*2] ,bkg):
                p[y*2+1,x*2] = mean4Pixelw( mip[y,x,:],144,
                                       mip[y,x-1] if x >0 else bkg, 48 if x >0 else 0,
                                       mip[y+1, x ] if y < mip.shape[0] -1 else bkg, 48 if y < mip.shape[0] -1 else 0,
                                       mip[y +1, x-1] if x > 0 and y < mip.shape[0] -1  else bkg, 16 if x >0 and y < mip.shape[0] -1 else 0)
            if ifequal(p[y*2+1,x*2+1] , bkg):
                p[y*2+1,x*2+1] = mean4Pixelw( mip[y,x,:],144,
                                       ...
                                       mip[y +1, x+1] if x < mip.shape[1] -1 and y < mip.shape[0] - 1  else bkg,
