...
def mesh_loop(mesh_v, mesh_f, mesh_n,mesh_n_f,mesh_tex,mesh_tex_f,vertexcolor,filepath, loopnum):
    ...
    result = meshlab_python.mesh_loop(
        filepath,
        mesh_v.tolist(), mesh_f.tolist(), mesh_n.tolist(), mesh_n_f.tolist(), mesh_tex.tolist(),mesh_tex_f.tolist(), vertexcolor.tolist(),
        int(loopnum))
    pass
...
def mesh_error_compare(input_mesh_path1,input_mesh_path2,output_mesh_path):
    ...
    write_full_obj(subdived_mesh2, subdived_mesh_f2, n_frame_aligned2, n_f_frame_aligned2,t_frame_aligned2, t_f_frame_aligned2,
                   color_3d,
                   output_mesh_path)
'''
input_mesh_path1  被比较的
...
def merge_splitedmesh(input_obj_path,outputobjpath):
    subdived_mesh, subdived_mesh_f, t_frame_aligned, t_f_frame_aligned, n_frame_aligned, n_f_frame_aligned = read_igl_obj(input_obj_path)
    ...
    write_full_obj(new_mesh_v,new_mesh_f,np.array([]),np.array([]),np.array([]),np.array([]),np.array([]),outputobjpath)
...
if __name__ == '__main__':
    ...
    if 0: #调整网格方位
        ...
        for k in b.fileList:
            ...
            write_full_obj(mesh_v=v, mesh_f=f, mesh_n=n,mesh_n_f=n_f,mesh_tex=t,mesh_tex_f=t_f,vertexcolor=vc,
                           filepath=out_dir+'/'+obj_name+'.obj', generate_mtl=False,verbose=False,img_name = 'default.png')
    if 0: # 输出颜色
        v_1, f_1, t_1, t_f_1, n_1, n_f_1 = read_igl_obj('D:\program files\R3DS\Wrap 3.3\Models\Dataset_Subject1\Dataset_Subject1\images\select_mesh\obj\convert/reconstruction_0246.obj')
        vc = readVertexColor('D:\program files\R3DS\Wrap 3.3\Models\Dataset_Subject1\Dataset_Subject1\images\select_mesh\obj\convert/reconstruction_0246.obj')
        ...
        write_full_obj(mesh_v=v_1, mesh_f=f_1, mesh_n=n_1, mesh_n_f=n_f_1, mesh_tex=t_2, mesh_tex_f=t_f_2, vertexcolor=vc,
    ...
    if 0:  #加入眼球
        ...
        b.FindFile(dirr=project_dir)
        import cv2
        f_mask = [1284,5212,2173,6085,770,4706,894,4827,895,4828,901,4834,902,892,888,4835,4909,916,4848,763,4699,765,893,977,4911,979,4919,984,4916,
                  2339,6250,6249,6245,2335,6248,2338,2337,6144,2232,6143,2231,6723,2814,2441,6351,6354,2444,
        ...
        for k in b.fileList:
            ...
            #vc = readVertexColor(k)
            write_full_obj(mesh_v=v2, mesh_f=f2, mesh_n=n, mesh_n_f=n_f, mesh_tex=np.array([]), mesh_tex_f=np.array([]), vertexcolor=np.array([]),
    ...
    if 1:
        mesh_error_compare(input_mesh_path1 ='D:\huayunhe/facewarehouse_new\FaceWarehouse_neutral_img_obj//Tester_1_pose_0.obj',
    ...
    if 0:
        mesh_error_compare2(input_mesh_path2='E:/workspace/tencent_test/sceneobj/guanyu_merge.obj', input_mesh_path1='E:/workspace/tencent_test/sceneobj/guanyucollider_merge.obj',
                           output_mesh_path='E:/workspace/tencent_test/sceneobj/guanyu_compare.obj')
    if 0:
        mesh_error_compare2(input_mesh_path2='E:/workspace/tencent_test/sceneobj/guanyu_merge_fillhole_union_clean.obj', input_mesh_path1='E:/workspace/tencent_test/sceneobj/guanyucollider_merge_union2.obj',
                           output_mesh_path='E:/workspace/tencent_test/sceneobj/guanyu_compare.obj')
    if 0:
