    ...
    for i in range(mu_F.shape[0]):
        ...
        for j in range(mu_B.shape[0]):
            ...
            while True:
                ...
                b1 = np.dot(invSigma_Fi , mu_Fi) + C*(alpha) * invsgma2
                b2 = np.dot(invSigma_Bj , mu_Bj) + C*(1-alpha) * invsgma2
                ...
                alpha = np.maximum(0, np.minimum(1, np.dot(  (np.atleast_2d(C).T-B).T , (F-B) )/np.sum((F-B)**2)))[0,0]
                ...
                L_F = (- ( np.dot( np.dot((F - np.atleast_2d(mu_Fi).T).T , invSigma_Fi),  (F - np.atleast_2d(mu_Fi).T))  ) / 2.)[0, 0]
                L_B = (- ( np.dot( np.dot((B - np.atleast_2d(mu_Bj).T).T , invSigma_Bj ), (B - np.atleast_2d(mu_Bj).T) ) ) / 2.)[0, 0]
...
def bayesian_matte(img, trimap, sigma=8, N=25, minN=10,fg_color =255,bg_color =0):
    img = img/255.0

    h, w, c = img.shape
    alpha = np.zeros((h, w))

    fg_mask = trimap == int(fg_color)
    bg_mask = trimap == int(bg_color)
...
def matting(ori_img_path,trimap_path):
    from time import  time
    img = scipy.misc.imread(ori_img_path)[:, :, :3]
    trimap = scipy.misc.imread(trimap_path, flatten='True')
    start = time()
    alpha = bayesian_matte(img, trimap,sigma=28, N=25, minN=10,fg_color =255,bg_color =127)
    print '耗时',time() -start
    return alpha
