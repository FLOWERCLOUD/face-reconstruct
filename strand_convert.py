...
def get_color_from_rad(rad):
    ...
    if theta < theta_range[1]:
        i = 0
        color = color_range[i] * (1 - (theta - theta_range[i]) / theta_step) + color_range[i + 1] * (
                theta - theta_range[i]) / theta_step
        ...
        i = 5
        color = color_range[i] * (1 - (theta - theta_range[i]) / theta_step) + color_range[0] * (
                theta - theta_range[i]) / theta_step
    else:
        color = color_range[0]
    return color
...
def generate_polystrip_mesh():
    ...
    for k in b.fileList:
        ...
        write_full_obj(v, f, n, nf, vt, vt_f, np.array([]),
                       prj_dir + '/convert_hair_texture/' + file_name + '_' + str(stand_num) + '.obj',
                       generate_mtl=True,
    ...
    for k in b.fileList:
        ...
        if 0:
            write_full_obj(v, f, n, nf, vt, vt_f, vt_color,
                           prj_dir + '/convert_hair_dir_new/' + file_name + '_' + str(stand_num) + '.obj',
                           generate_mtl=False,
                           verbose=False, img_name='hair2.png')
        elif 0:
            write_full_obj(v, f, n, nf, vt, vt_f, vt_color,
                           prj_dir + '/convert_hair_no_constraint/' + file_name + '_' + str(stand_num) + '.obj',
                           generate_mtl=False,
                           verbose=False, img_name='hair2.png')
        elif 1:
            write_full_obj(v, f, n, nf, vt, vt_f, vt_color,
                           prj_dir + '/convert_hair_helix/' + file_name + '_' + str(stand_num) + '.obj',
                           generate_mtl=False,
                           verbose=False, img_name='hair2.png')
        else:
            write_full_obj(v, f, n, nf, vt, vt_f, vt_color,
...
def generta_segment_map_batch(output_name, use_vertex_color=False):
    ...
    if 0:
        ...
        for i in range(0, face_landmark_3d.shape[0]):
            ...
            merge_head_body_v, merge_head_body_v_f = add_vertex_faces(merge_head_body_v, merge_head_body_v_f, plane_v,
                                                                      plane_f)
    ...
    for k in b.fileList:
        ...
        if 0:
            Mesh_render_to_image_withmy_bbox(hair_file_dir + output_name + file_name + '.png', mesh, image_width,
                                             image_height, bbox)
        write_full_obj(merge_v, merge_v_f, np.array([]), np.array([]), np.array([]), np.array([]), merge_color,
                       hair_file_dir + output_name + file_name + '.obj')
        bbox_list = [float(bbox.min[0]), float(bbox.min[1]), float(bbox.min[2]), float(bbox.max[0]), float(bbox.max[1]),
                     float(bbox.max[2])]
...
def seg_shape_similarity():
    ...
    for i in range(0, len(emd_array)):
        ...
        if emd_array[i].flip:
            img = img[:, ::-1, :]
        cv2.imwrite('E:/workspace/dataset/hairstyles/hair/convert_hair_dir/seg_bin/' + source + '_similarity' + str(
            use_emd) + '/'
                    + str(i) + '.png'
                    , img)
...
def get_similar_hair_from_database_wrapper(input_seg_img, input_dir_img, seg_database_path, seg_database_input_dir,
    ...
    cv2.imwrite(out_put_dir + 'best' + str(
        use_emd) + '/' + 'dir_' + 'useemd_' + str(use_emd) + '_' + str(min_dis_corr_idx) + '_corr_' + emd_array[
                    min_dis_corr_idx].name + '_flip_' + str(
    ...
    for i in range(0, len(emd_array)):
        ...
        cv2.imwrite(out_put_dir + 'shape_similarity' + str(
            use_emd) + '/' + 'dir_' + 'useemd_' + str(use_emd) + '_' + str(i) + '_corr_' + emd_array[
                        i].name + '_flip_' + str(
...
def build_hair_for_img_simgle(object_name, input_ori_img_file, input_seg_img_file, input_dir_img_file,
    ...
    write_full_obj(v_frame_align_to_image, f_frame_init, vn_frame_align_to_image, f_frame_init, t_frame_init,
                   t_f_frame_init, v_color_caculate, out_put_dir + '/' +
                   'frame_aligned_to_image' + '.obj')
    write_full_obj(v_frame_align_to_init, f_frame_aligned, n_frame_aligned, n_f_frame_aligned, t_frame_aligned,
                   t_f_frame_aligned, np.array([]), out_put_dir + '/' +
    ...
    for i in range(0, len(emd_array)):
        ...
        if flip:
            v_hair[:, 0] = -v_hair[:, 0]
            f_hair = f_hair[:, ::-1]
        v_hair_transformed = Scale * np.dot(R, (
                (Scale_local * np.dot(R_Local, v_hair.T)).T + T_local).T).T + T  # +np.array([0,-30,10])
        if i < 30:
...
def get_hair_wrapper_single(input_strands, ourput_dir, name):
    ...
    for i in range(0, verts.shape[0]):
        ...
        verts[i, :] = [x, y, z]
    write_full_obj(mesh_v=verts, mesh_f=faces, mesh_n=np.array([]), mesh_n_f=np.array([]), mesh_tex=np.array([]),
                   mesh_tex_f=np.array([]),
                   vertexcolor=np.array([]), filepath=ourput_dir + name + 'range_1_10' + str(point_rad) + '.obj',
                   generate_mtl=False, verbose=False, img_name='default.png')
    ...
    for i in range(0, len(connect_list)):
        ...
        for j in range(0, len(c_list), 1):
            ...
            if len(phis) == 0:
                print 'phis error'
                continue
            v, f, vt, vt_f, v_color = convert_2_mesh(phis, thetas, points, constraint_dir=constraint_dir, radius=0.01,
                                                     step=step, if_fix_dir=True)
            all_v, all_f = add_vertex_faces(all_v, all_f, v, f)
...
def generate_strip():
    ...
    for k in b.fileList:
        ...
        strip_mesh = generate_strip_from_connect_list(input_strands=input_strands, connect_list=connect_list,
                                                      wrap_mesh=wrap_mesh, horizontal_map=horizontal_map)
        write_full_obj(strip_mesh['mesh_v'], strip_mesh['mesh_f'], strip_mesh['mesh_n'], strip_mesh['mesh_n_f'],
                       strip_mesh['mesh_vt'],
