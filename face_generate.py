...
landmark_face = np.array([[57, 8, 9], [57, 9, 56], [56, 9, 10], [56, 10, 55], [55, 10, 11], [55, 11, 54], [54, 11, 12],
                          [54, 12, 13], [54, 13, 35], [13, 14, 35], [14, 47, 35], [14, 46, 47], [14, 15, 46],
                          [15, 45, 46],
                          [15, 16, 45], [16, 26, 45], [26, 25, 45], [25, 44, 45], [25, 24, 44], [24, 43, 44],
                          [24, 23, 43],
                          [23, 22, 43], [22, 42, 43], [22, 27, 42], [27, 28, 42], [28, 29, 42], [42, 29, 47],
                          [47, 29, 35],
                          [29, 30, 35], [30, 34, 35], [30, 33, 34], [33, 51, 52], [33, 52, 34], [34, 52, 35],
                          [35, 52, 53],
                          [35, 53, 54], [51, 62, 52], [52, 62, 63], [52, 63, 53], [53, 63, 64], [53, 64, 54],
                          [64, 55, 54],
                          [64, 65, 55], [65, 56, 55], [65, 66, 56], [66, 57, 56],
                          [57, 7, 8], [57, 58, 7], [58, 6, 7], [58, 59, 6], [59, 5, 6], [59, 48, 5], [48, 4, 5],
                          [48, 3, 4],
                          [31, 3, 48], [31, 2, 3], [31, 40, 2], [40, 41, 2], [41, 1, 2], [36, 1, 41], [36, 0, 1],
                          [36, 17, 0],
                          [36, 18, 17], [36, 37, 18], [37, 19, 18], [37, 38, 19], [19, 38, 20], [20, 38, 21],
                          [21, 38, 39],
                          [21, 39, 27], [27, 39, 28], [28, 39, 29], [39, 40, 29], [40, 31, 29], [29, 31, 30],
                          [30, 31, 32],
                          [30, 32, 33], [31, 48, 49], [31, 49, 50], [31, 50, 32], [32, 50, 33], [33, 50, 51],
                          [48, 60, 49],
                          [49, 60, 61], [49, 61, 50], [50, 61, 62], [50, 62, 51], [48, 59, 60], [59, 67, 60],
                          [67, 59, 58],
                          [67, 58, 66], [66, 58, 57]])
...
def generate_face(vrn_object_dir, object_name, out_put_dir, project_dir,
                  ...
                  frame_lmk_emb_path='./data/lmk_embedding_intraface_to_flame.pkl', use_3d_landmark=False,
                  pre_result=None):
    ...
    model = load_model(
        frame_model_path)  # the loaded model object is a 'chumpy' object, check https://github.com/mattloper/chumpy for details
    # landmark embedding
    ...
    mesh_v, mesh_f, parms = fit_lmk3d(lmk_3d=target_lmk_v,  # input landmark 3d
                                      model=model,  # model
                                      mesh_faces=model.f,
                                      lmk_facevtx_idx=face_select_lmk, lmk_bodyvtx_idx=body_select_lmk,
                                      # landmark embedding
                                      ...
                                      output_dir=out_put_dir, use_3d_landmark=use_3d_landmark,
                                      pre_result=pre_result)  # options
    ...
    write_full_obj(subdived_mesh, subdived_mesh_f, n_frame_aligned, n_f_frame_aligned, t_frame_aligned,
                   t_f_frame_aligned, v_color, out_put_dir + '/subface_with_texture.obj')
...
def fit_lmk3d(lmk_3d,  # input landmark 3d
    ...
    from test.para_texture_buid import add_texture_and_write
    ...
    print global_rotate
    parms = {'trans': g_trans_2d.r, 'pose': model.pose.r, 'betas': model.betas.r, 'scale': g_scale.r,
...
def shape_from_shading(input_model_dir, ori_img, seg_img, output_dir):
    ...
    for i in range(0, len(select_vtx_idx)):
        ...
        if 1:
            ...
            kk = ch.concatenate(
                [vector1[1] * vector2[2] - vector2[1] * vector1[2], vector1[2] * vector2[0] - vector2[2] * vector1[0],
    ...
    R_vec = []
    for i in range(0, len(select_vtx_idx)):
        y_nij = ch.concatenate((ch.array([1]), select_normal_list[i][0], select_normal_list[i][1],
                                select_normal_list[i][2]))  # [1,nx,ny,nz]
...
def extract_detail(subdived_mesh_path, ori_img, seg_img, output_dir):
    from fitting.util import build_adjacency, get_vertex_normal, sample_intensity_from_img
    import cv2
    import math
    ...
    write_full_obj(subdived_mesh, subdived_mesh_f, np.array([]), np.array([]), np.array([]), np.array([]),
                   intensity_color, output_dir + '/mesh_with_color.obj')
    ...
    write_full_obj(subdived_mesh, subdived_mesh_f, np.array([]), np.array([]), np.array([]), np.array([]), np.array([]),
                   output_dir + '/detail_mesh' + str(dt) + '.obj')
...
def extract_detailnew(input_obj, output_dir, objname, dt1=-200000):
    ...
    import math
    import geometry as geom
    from shortfunc import adjmat_to_laplacian2, mldivide
    import scipy.sparse as sparse
    subdived_mesh, subdived_mesh_f, t_frame_aligned, t_f_frame_aligned, n_frame_aligned, n_f_frame_aligned = read_igl_obj(
        input_obj)
    ...
    write_full_obj(subdived_mesh, subdived_mesh_f, np.array([]), np.array([]), np.array([]), np.array([]), vcolor,
                   output_dir + '/' + objname + '_detail_mesh_threshod_' + str(threshold) + ' ' + str(
                       dt) + 'scale_' + str(scale) + '.obj')
...
def fit_lmk3d_2(target_v, target_f, target_lmk_3d_idx,  # input landmark 3d
    ...
    for i in range(0, 20):
        ...
        objectives_pose = {}
        objectives_pose.update(
            {'p2p_error': p2p_error, 'face_lmk': face_lmk_err, 'shape': shape_err, 'expr': expr_err, 'pose': pose_err})
        if optim_expression:
            ch.minimize(fun=objectives_pose,
    ...
    global_rotate = cv2.Rodrigues(global_rotate_pose)[0]
    print global_rotate
    parms = {'trans': g_trans_3d.r, 'pose': model.pose.r, 'betas': model.betas.r, 'scale': g_scale.r,
             'global_rotate': global_rotate}
    print parms
    g_v = g_scale.r * model.r[:, :]
    g_v[:, :] = g_v[:, :] + g_trans_3d.r
    ...
    for y in range(0, height_map.shape[0]):
        for x in range(0, height_map.shape[1]):
            if valid_map[y, x]:
                ...
                y_nij = ch.concatenate(
                    (ch.array([1]), normal_map[y, x, 0], normal_map[y, x, 1], normal_map[y, x, 2]))  # [1,nx,ny,nz]
...
def test_pncc():
    ...
    write_full_obj(pncc, f_frame_aligned, np.array([]), np.array([]), np.array([]), np.array([]), np.array([]),
                   'E:\workspace\dataset\hairstyles/test_pncc.obj')
...
def test_scale_to_pncc():
    ...
    result = meshlab_python.Mesh_render_to_image_withmy_bbox(
        'E:/workspace/dataset/hairstyles/2d_hair/result/A13010436665609/builded_hair/get_z_buffer.png',
...
def fitting_frame_to_mesh_by3d_landmark(target_v, target_f, target_landmark_idx, out_put_dir,
                                        frame_model_path='./models/generic_model.pkl',
...
def generate_mesh_with_color():
    ori_img = cv2.imread('E:\workspace/vrn_data/bgBlue/A13010436665609/generate_face/' + 'A1301074625120A.jpg',
...
def genetate_expression_single(shape_model, vrn_object_dir, object_name, project_dir, out_put_dir,
                               frame_model_path='./models/male_model.pkl', use_3d_landmark=False, preresult=None):
    ...
    while index_i < 10:
        R, s, t = optRSTfromShape_t(faceshape, face, R, s, t, featurePointxy, lmk_face_idx, lmk_b_coords,
                                    face_select_lmk, pre_R, pre_s, pre_t)
        beta = optexpressFromRST_t(shape_model, face, R, s, t, featurePointxy, lmk_face_idx, lmk_b_coords,
                                   face_select_lmk, pre_beta, boundShape)
...
def generate_expression_simp(pre_result, vrn_object_dir, object_name, project_dir, out_put_dir,
    ...
    from test.hello_world import render_to_image
...
def generate_expression(pre_result, vrn_object_dir, object_name, project_dir, out_put_dir,
                        frame_model_path='./models/male_model.pkl'):
    # 先生成blendshape
    safe_mkdirs(out_put_dir)
    from smpl_webuser.serialization import load_model
...
if __name__ == '__main__':
    ...
    import cv2
    import os
    from fitting.util import test_frame_mesh_landmark, read_int, \
    ...
    pncc_landmark = np.array(
        [21609, 21806, 22051, 22221, 22387, 23072, 23237, 23446, 23738, 24060, 8295, 8304, 8311, 8316, 6393, 7171, 8329,
         9615, 10775,
         1700, 3755, 4658, 6476, 4676, 3644, 10079, 11103, 12521, 14331, 12667, 11635, 5133, 5897, 7312, 8341, 9369,
         10791, 11705, 10173,
    ...
    if 0:
        extract_detail(subdived_mesh_path=input_model_dir + '/' + 'plane_subdiv2.obj', ori_img=ori_img,
                       seg_img=seg_img_binary,
    ...
    if 0:
        extract_detailnew(
            input_obj='E:/workspace/vrn_data/bgBlue/A13010436665609/generate_face/triangle_mesh_1_smooth2.obj',
            output_dir='E:/workspace/vrn_data/bgBlue/A13010436665609/generate_face/test_smooth/')
    if 0:
        extract_detailnew(
    ...
    if 0:
        ...
        pre_result = generate_face(frame_model_path='./models/generic_model.pkl',
                                   vrn_object_dir='E:/workspace/vrn_data/expression_hair/' + obj_name + '/',
                                   object_name=obj_name,
                                   out_put_dir='E:/workspace/vrn_data/expression_hair/' + obj_name + '/generate_face/',
                                   use_3d_landmark=False)
        objnames = ['useful-faces-picture-id108686357_01', 'useful-faces-picture-id108686357_02',
                    'useful-faces-picture-id108686357_03', 'useful-faces-picture-id108686357_04',
                    'useful-faces-picture-id108686357_05', 'useful-faces-picture-id108686357_06',
                    'useful-faces-picture-id108686357_07', 'useful-faces-picture-id108686357_08',
                    'useful-faces-picture-id108686357_10',
                    'useful-faces-picture-id108686357_11', 'useful-faces-picture-id108686357_12',
                    'useful-faces-picture-id108686357_13'
                    ]
        objnames = ['useful-faces-picture-id108686357_14', 'useful-faces-picture-id108686357_15',
                    'useful-faces-picture-id108686357_16', 'useful-faces-picture-id108686357_17',
                    'useful-faces-picture-id108686357_18']
        objnames = ['collage-young-stylish-man-expressions-white-background-35379705_01',
                    'collage-young-stylish-man-expressions-white-background-35379705_02',
                    'collage-young-stylish-man-expressions-white-background-35379705_03',
                    'collage-young-stylish-man-expressions-white-background-35379705_04',
                    'collage-young-stylish-man-expressions-white-background-35379705_06'
                    ]
    ...
    if 0:
        ...
        if 1 and os.path.exists(input_model_dir + '/face_result.pkl'):
            ...
            pre_result = generate_face(frame_model_path='./models/male_model.pkl',
                                       ...
                                       out_put_dir=project_dir + obj_name + '/generate_face/',
                                       use_3d_landmark=False)
        generate_expression(pre_result=pre_result, vrn_object_dir=project_dir + obj_name + '/', object_name=obj_name,
                            project_dir=project_dir, out_put_dir=project_dir,
                            frame_model_path='./models/male_model.pkl')
    ...
    if 0:  # generate blend shape for natural shape
        ...
        for k in b.fileList:
            ...
            if os.path.exists(input_model_dir + '/face_result.pkl'):
                pre_result = load_binary_pickle(filepath=input_model_dir + '/face_result.pkl')
            generate_expression_simp(pre_result=pre_result, vrn_object_dir=project_dir + obj_name + '/',
                                     object_name=obj_name, project_dir=project_dir, out_put_dir=project_dir,
                                     frame_model_path='./models/male_model.pkl')
        project_dir = 'E:\workspace/vrn_data\hairstyle/women/'
        project_dir = 'E:\workspace/vrn_data\paper_select1/women/'
        project_dir = 'E:\workspace/vrn_data\paper_compare/neatrual_1/women/'
    ...
    if 0:
        ...
        igl.writeOBJ('E:\workspace/vrn_data/' + '/pointlist_landmark' + '.obj',
                     igl.eigen.MatrixXd(landmark3d.astype('float64')),
                     igl.eigen.MatrixXi(landmark_face.astype('intc')))
    if 0:
    ...
    if 1:
        ...
        if 0:
            ...
            write_full_obj(generate_mesh, generate_mesh_f, generate_mesh_n, n_f_frame_aligned, t_frame_aligned,
                           t_f_frame_aligned, np.array([]),
        ...
        seg_img_binary = cv2.resize(seg_img_binary, (int(ori_img.shape[1]), int(ori_img.shape[0])),
        ...
        if 0:
            ...
            write_full_obj(subdived_mesh, subdived_mesh_f, n_frame_aligned, n_f_frame_aligned, t_frame_aligned,
                           t_f_frame_aligned, vcolor, project_dir + '/generate_face/' + 'subdiv_with_color' + '.obj')
        # 生成深度图网格
        if 0:
            subdived_mesh, subdived_mesh_f, t_frame_aligned, t_f_frame_aligned, n_frame_aligned, n_f_frame_aligned = read_igl_obj(
            ...
            render_img, z_buffer_img = get_z_buffer(subdived_mesh, subdived_mesh_f, ori_img,
                                                    output_render_img=project_dir + 'render_img_0.png', vcolor=vcolor)
    ...
    if 0:
        ...
        write_full_obj(optim_v, subdived_mesh_f, n_frame_aligned, n_f_frame_aligned, np.array([]), np.array([]),
    ...
    if 0:
        ...
        subdived_mesh, subdived_mesh_f, t_frame_aligned, t_f_frame_aligned, n_frame_aligned, n_f_frame_aligned = read_igl_obj(
            'D:\mproject/face-reconstruct/texpc/new_registration/fitting_frame_face.obj')
        ...
        write_full_obj(subdived_mesh, subdived_mesh_f, n_frame_aligned, n_f_frame_aligned, np.array([]), np.array([]),
                       color_3d, 'D:\mproject/face-reconstruct/texpc/new_registration/fitting_frame_face_error' + str(
                vmax) + '.obj')
    if 0:
        subdived_mesh, subdived_mesh_f, t_frame_aligned, t_f_frame_aligned, n_frame_aligned, n_f_frame_aligned = read_igl_obj(
        ...
        mesh_loop(subdived_mesh, subdived_mesh_f, n_frame_aligned, n_f_frame_aligned, t_frame_aligned,
                  t_f_frame_aligned, vtx_color,
    ...
    if 0:
        ...
        if os.path.exists(input_model_dir + '/face_result.pkl'):
            ...
            pre_result = generate_face(frame_model_path='./models/male_model.pkl',
                                       ...
                                       out_put_dir=project_dir + obj_name + '/generate_face/',
                                       use_3d_landmark=False)
        generate_expression(preresult=pre_result, vrn_object_dir=project_dir + obj_name + '/', object_name=obj_name,
                            project_dir=project_dir, out_put_dir=project_dir,
                            frame_model_path='./models/male_model.pkl')

    if 0:
        color_sum = extract_para_texture(
            input_texture_img='E:\workspace/vrn_data\paper_select1\man/004_1\generate_face/subdiv1_full.png',
    ...
    if 0:
        ...
        model = load_model(
            './models/male_model.pkl')  # the loaded model object is a 'chumpy' object, check https://github.com/mattloper/chumpy for details
        # landmark embedding
